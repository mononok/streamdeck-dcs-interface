...
TEST(DcsInterfaceTest, dcs_reset_command_on_construction) {
    DcsConnectionSettings connection_settings = {"1908", "1909", "127.0.0.1", "127.0.0.1" };
    DcsSocket mock_dcs(connection_settings.ip_address, connection_settings.ip_address_partner, connection_settings.tx_port, connection_settings.rx_port);
  ...
  public:
    DcsInterfaceTestFixture()
        : // Mock DCS socket uses the reverse rx and tx ports of dcs_interface so it can communicate with it.
          mock_dcs(connection_settings.ip_address, connection_settings.ip_address_partner, connection_settings.tx_port, connection_settings.rx_port),
